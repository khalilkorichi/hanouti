...
- **Manager menu + customizable shortcuts (v1.3.1)**: The user card in the top header is now a real Popover (`components/Layout/AdminMenu.tsx`) with avatar, name, email, role chip, quick stats (store name, business type, last login, current theme), and an action list: Settings, Profile (deep-links to Settings via `navigate('/settings', { state: { section } })`), Change Password (mounts existing `Auth/ChangePasswordDialog`), Shortcuts dialog, theme toggle, quick backup, About modal, and Logout (clears token + `/login`). The shortcuts registry lives in `store/shortcutsStore.ts` (Zustand `persist`, key `hanouti-shortcuts`) and contains all built-in POS + global bindings plus user-added custom bindings to a fixed action catalogue. `Sales.tsx` and `GlobalBarcodeShortcut.tsx` consume their combos through `useShortcutCombo(id)`, so customisations apply live without restart. `components/Shortcuts/ShortcutsManagerDialog.tsx` is the unified View/Customize dialog (key capture, conflict detection via `findConflict`, restore-defaults). `components/Shortcuts/GlobalCustomShortcuts.tsx` is mounted once in `MainLayout` and dispatches custom bindings (navigation, theme toggle, open shortcuts dialog, jump to backup section). The shortcuts dialog is mounted **only** in `MainLayout` and exposed to `AdminMenu` via an `onOpenShortcuts` prop chain (`MainLayout → Header → AdminMenu`) so there is exactly one instance.
- **Deep Appearance customization (v1.4.0)**: Settings → المظهر got new cards: number/money display (`hideTrailingZeros`, `decimalPlaces` 0/2/3, `thousandSeparator`, `currencySymbol`, `currencyPosition` before/after, with live preview), font scale slider 80–130 % + Arabic font family picker (Cairo/Tajawal/Almarai/Noto Kufi/Noto Naskh/IBM Plex) + bold-numbers switch, general preferences (default landing page, default sidebar collapsed state, show product images, toast position 6 corners, date format, high contrast), plus a "restore defaults" button. State + persistence + dynamic Google-Fonts `<link>` injection live in `contexts/ThemeContext.tsx`; reusable formatters in `utils/format.ts` (`formatMoney/formatNumber/formatDate` + `useFormatters()` hook) replace hand-rolled `.toFixed(2) + ' دج'` / `toLocaleString('ar-DZ')` patterns across `Sales/CartPanel.tsx`, `Sales/Receipt.tsx`, `pages/Reports.tsx`, `pages/Inventory.tsx`, `pages/SalesList.tsx`, `pages/Dashboard.tsx`, `pages/Customers.tsx` (CSV exports kept raw for Excel parity). `NotificationStack` reads `toastPosition` from `AppThemeProvider` for live positioning, `MainLayout` uses `sidebarCollapsedDefault` as the initial fallback when no explicit user override is in localStorage, `App.tsx` routes `/` and `*` through `defaultPage`, and `index.css` adds `.app-bold-numbers` (tabular-nums + bolder numeric grid cells) and `.app-high-contrast` rules toggled on `<html>` by `ThemeContext`.
- **Activity Log + Restore Points (v1.3.0)**: Settings has a new `سجل النشاطات` section (`ActivityLogCard.tsx`) showing every mutation (products, categories, inventory, sales, customers, payments, factory-reset, backup import/restore) with severity chips (info/success/warning/critical), filters (action, severity, time range), and a clear-all action. The audit row is committed via a sibling `SessionLocal` inside `services/activity_service.py` so it survives request rollbacks. The auto-backup loop in `main.py` (`_auto_backup_loop`) ticks every 60s, re-reads the user-configured interval (`KEY_AUTO_BACKUP_INTERVAL_MIN`, 5min–1week, default 1 week) and `KEY_AUTO_BACKUP_ENABLED` from `app_settings` on every tick so schedule changes take effect within ~60s without restart. Manual restore points support a user-supplied name + note stored in snapshot `meta` (outside the data checksum, so integrity validation still works); UI in `RestorePointsCard.tsx`. New endpoints: `GET/PUT /settings/auto-backup`, `GET /activity/list`, `GET /activity/actions`, `DELETE /activity/clear`, `POST /backup/manual-snapshot`.
