...
> **ملاحظة:** كل عمليات البيع تتم داخل Transaction  - السبب: ضمان سلامة البيانات ومنع التكرار.

## 10) نقطة البيع (Phase 04 - تم الإنجاز ✅)
تم بناء واجهة نقطة البيع (POS) لتكون سريعة وتفاعلية.

### القرارات التقنية:
- **Drag & Drop:** استخدام مكتبة `@dnd-kit` لسحب المنتجات إلى السلة.
  - السبب: تجربة مستخدم عصرية وسهلة، خاصة للأجهزة اللوحية.
- **إدارة السلة (State):** استخدام `Zustand` مع `persist`.
  - السبب: حفظ حالة السلة محليًا (LocalStorage) لمنع فقدان البيانات عند تحديث الصفحة، وأداء أفضل من Context API للتحديثات المتكررة.
- **الطباعة:** استخدام `react-to-print` مع مكون `Receipt` مخفي.
  - السبب: تحكم كامل في تنسيق الفاتورة (HTML/CSS) وضمان دعم العربية (RTL) بشكل ممتاز مقارنة بتوليد PDF في المتصفح.
- **Grid System:** استخدام MUI Grid v2 (`size` prop).
  - السبب: التوافق مع أحدث إصدارات MUI وتحسين الاستجابة.
